...
| B.7 | Valor de compra (purchase value) do Google Ads/GA4 subavaliado na Mellmak | Ticket médio implícito Google ≈26€/compra vs. Meta ≈80€/compra (mesma loja, mesmo período); na Forte Store os dois batem certo (≈67€ vs. ≈67€). Aponta para configuração incorreta do parâmetro `value` no evento de compra do GA4/Google Ads da Mellmak, não para diferença real de eficiência | "Valor conv./custo" do Google para a Mellmak (10,02 em julho, 13,87 em 1-15 ago) está artificialmente inflacionado — não usar em decisões nem publicar externamente sem esta ressalva (ver Regra 5 da constituição). **Correção de proveniência (21-08, dona da conta):** este NÃO é um achado desta sessão — foi identificado pela própria gestora da conta antes de junho de 2026, com o bug a existir desde 2023 sem nunca ter sido detetado por ninguém até então. **Já está combinado com a Redicom que a correção acontece após o período de saldos** (não podia ser corrigido em junho, quando foi descoberto, por estar em plena campanha ativa — mudar tracking a meio de saldos arriscaria contaminar dados de uma campanha em curso). Estado real: **diagnosticado e com correção agendada**, não "em aberto" nem "por investigar". Corresponde aos itens 6/7 da auditoria técnica de junho (ver `01_data_sources.md`), que documentou o mesmo problema, não o descobriu pela primeira vez. |
...
| C.16 | Corrigir o parâmetro `value` do evento de compra GA4/Google Ads na Mellmak (B.7) | fora do Neptune COS | **agendado** — combinado com a Redicom para logo após o fim do período de saldos atual (não antes, por risco de contaminar dados de campanha em curso); validar depois nas duas plataformas |
| C.17 | Reavaliar split de investimento Meta/Google da Mellmak (90%/10%) depois de corrigido o B.7 | Fase 6 (Decision Engine) | bloqueado por C.16 — validação externa recebida em 21-08 (peer do setor, Lolita Moda) reforça o achado B.7 por triangulação independente (AOV Google Mellmak ~26-27€ vs. ~55-68€ observado por terceiros no mesmo canal). Se o valor real de compra for ~80€ (como no Meta da própria Mellmak), o ROAS Google real pode rondar 30-40x vs. 4,5-5,1x do Meta — indício de possível má alocação do investimento a favor do canal menos eficiente. Correção (21-08, dona da conta Mellmak/Forte Store, 20 anos no setor e na plataforma desde 2013): não existem lojas físicas — Mellmak e Forte Store são 100% online, elimina a hipótese do Pablo sobre "visibilidade de lojas físicas"; e o público de Mellmak/Forte Store comporta-se de forma diferente do mercado espanhol, o que é uma variável legítima adicional a considerar na leitura do split (ver `03_business_rules.md` variáveis de benchmark). **O split atual coexiste com a meta formal já superada — não é evidência de erro, é evidência de que ainda não sabemos se há uma alocação melhor por trás de um número comprovadamente avariado.** Não decidir sem antes: (1) corrigir o tracking, (2) analisar conversões assistidas no GA4 corrigido, (3) testar com deslocamento incremental de orçamento, nunca um corte grande de imediato, e sempre como proposta ao Nuno |
